...
public class UploadWindow extends Window {
	...
	public UploadWindow(long idCaso) {
		...
		okButton.addListener(new ButtonListenerAdapter() {
			public void onClick(Button button, EventObject e) {
				...
				try {
					...
					MessageBox.alert("Sua imagem foi inserida com sucesso.");
					close();
				} catch (Exception exc) {
					MessageBox.alert("O arquivo não é uma imagem.");
				}
			}
		});
		return okButton;
	...
	private void validateImage(String path) throws Exception{
		...
		if(!this.isValidExtension(extension)){
			MessageBox.alert("O arquivo não é uma imagem e não pode ser inserido.");
		}
	}
